...
func (client *Client) handleCuratorClaim(c *gin.Context) {
	...
	if resp.StatusCode != 200 {
		...
		// Parse YouTube error for user-facing message
		var ytErr struct {
			Error struct {
				Code    int    `json:"code"`
		...
		json.Unmarshal(body, &ytErr)
		c.JSON(resp.StatusCode, gin.H{
			"error":          fmt.Sprintf("YouTube API error (%d): %s", resp.StatusCode, ytErr.Error.Message),
			"youtube_status": resp.StatusCode,
		})
		return
	}
	...
	for _, item := range ytResp.Items {
		...
		allowListed := item.ID == "UCrqgghsZVhGvddpsqxBc3XA"
		var count int
